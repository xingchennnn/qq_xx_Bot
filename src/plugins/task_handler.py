...
from .common import TARGET_QQ, get_best_bounty, wait_and_resume_sect, wait_and_settle_bounty
...
async def seclusion_out(bot: Bot, group_id: int):
    ...
    current_seclusion = task_states[group_id]["seclusion_state"]
    ...
    if current_seclusion == seclusionState.SECT_RUNNING:
        ...
        task_states[group_id]["seclusion_state"] = seclusionState.IDLE
...
async def start_bounty_task(bot: Bot, group_id: int, task_type=TaskType.BOUNTY_ONLY): # 启动悬赏令流程
    ...
    task_states[group_id] = {
        "type": task_type,
        "phase": TaskPhase.BOUNTY,
        "sect_state": SectState.IDLE,
        "bounty_state": BountyState.WAIT_LIST,
        "stop_after": False,
        "seclusion_state": seclusionState.UNKNOWN
    }
...
async def handle_task_reply(bot: Bot, event: GroupMessageEvent): # 处理任务相关回复
    ...
    if current_phase == TaskPhase.SECT:
        ...
        if state_data["sect_state"] == SectState.WAITING_BOUNTY: # 等待悬赏令阶段
            if "悬赏壹" in msg_text or "结算" in msg_text:
                ...
                state_data["sect_state"] = SectState.RUNNING
            return
        ...
        if "当前任务" in msg_text or "任务查看" in msg_text:
            if "除魔令" in msg_text or "狩猎邪修" in msg_text or "宗门密令" in msg_text:
                ...
                await bot.send_group_msg(group_id=group_id, message="检测到非邪修任务，5分钟后重新接取...")
                
                async def retry_sect_task():
                    await asyncio.sleep(305)
                    if group_id in task_states:
                        task_states[group_id]["sect_state"] = SectState.RUNNING
                        await bot.send_group_msg(group_id=group_id, message=MessageSegment.at(TARGET_QQ) + " 宗门任务接取")
                
                asyncio.create_task(retry_sect_task())
            return
        ...
        if "道友兴高采烈的出门做任务" in msg_text: #气血不足
            await asyncio.sleep(2)
            # 气血不足时，需要先出关（如果闭关了），再闭关回血
            # 这里我们假设如果 seclusion_state 是 UNKNOWN，可能需要先探测
            # 但通常气血不足是在任务中触发的，此时 seclusion_state 应该是 IDLE
            # 如果是 IDLE，我们需要先闭关回血
    ...
    # ==================== 阶段二：悬赏令 ====================
    elif current_phase == TaskPhase.BOUNTY:
